...
添加还代码按ctrl+s，CMake就会自动开始Config。
...
难道要全部复制过来？如果全部复制过去，可以发现确实可以运行的，或换一个思路
...
当然如果你上网搜索，可以发现，qt官方有个工具可以把所以的dll自动复制过来
...
首先看CMakeLists.txt，有一个硬编码的路径，如果把项目发给别人，那人的QT库路径和我们一样吗？很难说
...
这个方法似乎比刚刚的那个好多了，只要加一个Path环境变量，别的都不要动，也没有硬编码路径
...
**这个方法的核心思想就是临时修改当前进程的环境变量Path，并且通过子进程继承环境变量，来达到刚刚修改系统环境变量的效果**
...
不过还是有问题的，如果用动态的qt库，使用release模式依旧会链接静态运行时
